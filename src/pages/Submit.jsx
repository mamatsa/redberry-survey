...
function Submit() {
  ...
  useEffect(() => {
    if (
      ...
      localStorage.getItem('progress') < 4
    ) {
      navigate(-1);
    }
  }, [navigate]);
  ...
  const onSubmitSurvey = async function () {
    ...
    const application = {
      ...
      will_organize_devtalk:
        localStorage.getItem('devtalks') === 'Yes' ? true : false,
    ...
    if (
      localStorage.getItem('covidContact') &&
      localStorage.getItem('covidContact') === 'Yes'
    ) {
    ...
    if (
      ...
      localStorage.getItem('vaccinated') === 'Yes'
    ) {
